...
export default async function UniversityLedgerPage({
  ...
  const totalPayable = ledgers.reduce((sum, ledger) => sum + ledger.amountPayable, 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Payable, paid, pending, and reconciliation view.
          </p>
        </div>
        <Link className="text-sm underline" href={`/api/exports/ledgers/university?${exportQuery}`}>
          Export CSV
        </Link>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
                  ...
                  return (
                    <TR key={ledger.id}>
                      <TD>
                        ...
                        <div className="text-xs text-zinc-500">{ledger.admission.course.name}</div>
                      </TD>
                      <TD>{ledger.university.name}</TD>
                      <TD>{formatINR(ledger.amountPayable)}</TD>
                      ...
                      <TD className="text-right">
                        ...
                        {ledger.payments.length > 0 ? (
                          <div className="mt-1 text-xs text-zinc-500">
                            ...
                            <a
                              className="underline"
                              href={`/api/ledgers/university/${ledger.payments[0].id}/slip`}
                              target="_blank"
                              rel="noreferrer"
                            >
                              PDF
          ...
          {ledgers.length === 0 ? (
            <div className="text-sm text-zinc-600">No ledger entries found.</div>
          ) : null}
        </CardContent>
      </Card>
    </div>
